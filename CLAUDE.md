...
**Primary collaborator:** Josh (jpcoleman12@gmail.com)
**Visibility:** Public (published 2026-09-17, decision D21; development history retained privately at BootstrapAI-mgmt/thermal-mesh-calculators-private)
...
This repo uses a custom push workflow via Python `urllib.request` → GitHub GraphQL `createCommitOnBranch` mutation. The auth token is obtained via `gh auth token` on the host machine (Windows MCP PowerShell). No local git clone is used in the VM.
